 ...
 *                        platform.deepseek.com / opencode.ai 会被 CORS 拦截，
 *                        本路由在宿主进程内转发（Node 原生 fetch），只放行
 *                        白名单 URL 和 authorization/accept 头，并做回环信任
 *                        栅栏（防 DNS rebinding 与跨站调用）。
 *   GET  /dshu/api/apikey 从宿主凭证解析 DEEPSEEK_API_KEY 与
 *                        OPENCODE_GO_API_KEY / OPENCODE_API_KEY，供面板自动
 *                        填充余额/额度查询；凭证只回给同源页面，不落盘、不进日志。
 *
