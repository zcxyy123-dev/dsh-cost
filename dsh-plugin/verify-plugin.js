...
const host = fs.readFileSync(files.host, 'utf8')
...
const checks = [
  ...
  ['Host source matches manifest', manifest.code && manifest.code.host === host],
  ['Client source matches manifest', manifest.code && manifest.code.client === client],
...
console.log(`Manifest SHA-256: ${digest(JSON.stringify(manifest))}`)
